...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 1,000 | 174.81 µs | 195.52 µs | 176.84 µs | 204.89 µs | 338.93 µs | **51.97 µs** | 99.19 µs | n/a |
| 100,000 | 23.957 ms | 24.945 ms | 20.862 ms | 24.878 ms | 43.524 ms | **55.29 µs** | 140.16 µs | n/a |
| 1,000,000 | 374.36 ms | 393.49 ms | 288.86 ms | 369.73 ms | **1.1728 s** | 51.63 µs | 138.43 µs | n/a |

**Verdict: checkpoint cost splits by exactly how much of the dataset each variant's checkpoint actually rewrites, with one clear implementation-level outlier.** wal_fsync/wal_buffered/snapshot_rebuild/snapshot_full all persist the *full current state* (all `n` records) on every checkpoint, and it shows — cost scales with `n`, from hundreds of µs at 1K to hundreds of ms at 1M, all four within a tight band of each other at every size (they're doing structurally the same "serialize everything, write it" work). **Hybrid is the one real outlier — 1.17 s at 1M, roughly 3× its closest full-state-persisting peer** — traced to `HybridStore::checkpoint` calling `self.state.clone()` before serializing (to build the `HybridSnapshot` value it writes), a full extra copy of the entire dataset that none of the other full-state variants perform; every other checkpoint call in this table serializes state it already owns outright. This is a real, measured cost of the current implementation, not an inherent property of the hybrid design.
...
| Size | wal_fsync | wal_buffered | snapshot_rebuild | snapshot_full | hybrid | mmap | lsm | redb |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 1,000 | 324.6 µs | 335.2 µs | 360.9 µs | 317.4 µs | 369.5 µs | 281.0 µs | **264.5 µs** | 1.060 ms |
| 100,000 | 77.86 ms | 74.13 ms | 77.48 ms | 74.74 ms | 80.24 ms | 76.60 ms | **59.10 ms** | 60.67 ms |
| 1,000,000 | 1.432 s | 1.544 s | 1.622 s | 1.562 s | 1.370 s | 1.305 s | **971.5 ms** | 987.8 ms |

**Verdict: at 100K/1M, every variant lands within roughly the same order of magnitude — because every variant's `open` pays the same dominant cost, rebuilding the breed/adjacency/position indexes from the externally-supplied `records`/`edges` (the "base dataset supplied externally" convention every variant shares — see `src/durability/mod.rs`'s module docs).** That shared O(n) rebuild dwarfs each variant's own persistence-specific I/O at these sizes: replaying a ~1,000-entry WAL, deserializing a full-state snapshot, or opening a `redb`/`LsmStore` file are each a comparatively small addition on top of it. LSM and `redb` come out consistently fastest at 100K/1M (they avoid deserializing a *second* full copy of the dataset the way the snapshot variants do, and their extra on-disk data — a small WAL or a handful of key/value pairs — is cheap to read), but the gap between fastest and slowest never exceeds ~13% at 1M — a second-order effect next to the shared index-rebuild cost, not a reason to pick one variant over another for load speed alone. `redb` is the one exception at 1,000 records: `Database::open`'s own fixed file-format overhead dominates at this size (1.06 ms vs. 260–370 µs for everyone else) but disappears into the noise once the shared rebuild cost dominates at 100K+.
...
| hybrid | 1,000 × 866.0 ns ≈ **0.87 ms** | Same as wal_buffered; `checkpoint()` exists but rewrites the *entire* dataset, not just this batch | 43.52 ms | ≈ 0.87 ms (process-crash) / ≈ 44.4 ms if a full checkpoint is forced | Same OS-write-back window unless a (disproportionately expensive) checkpoint is forced |
...
- **If a small, OS-bounded loss window is acceptable in exchange for ~1,000× cheaper writes**: **WAL buffered (variant 2) over Hybrid (variant 5), for this crate's specific shape.** Hybrid's whole premise — bounding *replay* cost via periodic snapshots — doesn't pay for itself here: 1,000-write replay cost is already in the same 970 ms–1.6 s band as every other variant (see the load table above, dominated by the shared index-rebuild step), so Hybrid's snapshot doesn't meaningfully shrink startup time, while its checkpoint is the single most expensive call measured in this entire section (1.17 s at 1M, itself inflated by an avoidable extra `state.clone()` — see the checkpoint section above). Hybrid remains architecturally sound and correctly tested (the cutoff-boundary replay test is real and passes); it just isn't earning its complexity at this crate's dataset shape and write volume.
- **If the mutation pattern genuinely matches "one small field changes, everything else is static" (which this crate's `age` field does)**: **mmap (variant 6) is the standout result of this whole pass** — cheaper per write than the non-durable in-memory baseline, and the cheapest explicit durability step of any variant by two to three orders of magnitude. This is not a general-purpose recommendation — it works specifically because only `age` needed to be mapped, per its own scope-down — but for exactly the shape this crate has, it's hard to beat.
- **Between the two snapshot-alone variants**, if a design without any WAL is chosen at all: **rebuild (variant 3) over full (variant 4).** Rebuild's checkpoint is marginally cheaper (20.9 ms vs. 24.9 ms at 100K) *and* self-corrects a future index-corruption bug on every load, where full would faithfully persist and reload the same corruption forever (see `snapshot_full.rs`'s own module docs) — full's only advantage (skipping the rebuild step) isn't worth giving up rebuild's safety net for a difference this small.
...
- **`HybridStore::checkpoint`'s extra `state.clone()`**: new this pass — identified as the specific cause of Hybrid's checkpoint cost being the outlier of the whole durability sweep (1.17 s at 1M vs. 289–393 ms for its full-state-persisting peers, see `## Durability`'s checkpoint section). A follow-up could serialize directly from `&self.state` (mirroring how `WalFsyncStore`/`SnapshotFullStore` already avoid an equivalent clone) rather than cloning first; not fixed this pass, since the task's working style is implement→test→benchmark→write up per variant, and this is a real, now-documented cost of the shipped implementation, not a correctness bug.
