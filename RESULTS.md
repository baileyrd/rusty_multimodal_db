...
- **The `scan_ages` 100K cache-miss crossover**: narrowed this pass, still open. SoA has fewer cache misses than Canonical+cache at 100K (opposite of the 1M result), despite both running the identical `.clone()`-of-packed-`Vec<u32>` code path at every size. Real memory-footprint measurements (`examples/memory_footprint.rs`) ruled out the simplest version of the "larger co-resident working set → more misses" hypothesis — Canonical+cache's footprint is larger than SoA's at *both* sizes, and relatively more so at 1M (+156%) than at 100K (+87%), which predicts the opposite of the observed crossover if footprint size alone were driving it. A finer-grained perf-counter bench (`benches/scan_ages_crossover.rs`, L1-data vs. last-level cache tiers) is built and ready to run on `baileyai`, but not run from this session — same PMU-access gap as everything else in the cache-miss section. Confirming the actual mechanism needs that run.
