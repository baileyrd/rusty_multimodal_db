...
v0.2.0), three (since v0.3.0), four (since v0.4.0), and five (since
v0.5.0) hops deep — run against three real, general-purpose databases:
SQLite, Postgres, and DuckDB. `get`/`scan_ages`/`neighbors_one_hop`/
`neighbors_two_hop` all mirror an in-repo benchmark shape exactly;
`neighbors_three_hop` through `neighbors_five_hop` do not — this crate's
own `bench_support` composes at most two hops (see ADR-0004), so those
three shapes are measured *only* here, not in-repo. See ADR-0015 for why
these three engines, why these query shapes, and the full accounting of
...
**v0.5.0 note**: a fifth later addition, same "no new ADR" reasoning —
`five_hop_neighbors` composes `four_hop_neighbors` one more round, same
private, bench-target-local placement. This round found that the v0.4.0
crossover was itself not stable: SQLite is closest again at 1,000,000
records at five hops, while a *different* crossover (Postgres closest)
appears at 100,000 records for the first time. The underlying Postgres
query plan is unchanged from v0.2.0 onward at every depth and size
tested (confirmed again via `EXPLAIN (ANALYZE, BUFFERS)`) — the
volatility is in how much that plan's buffer I/O grows per hop at a
given size, not in which plan runs. See `RESULTS.md`'s `### Graph
traversal, five-hop` subsection for the full account.
...
- Not six-or-more-hop graph traversal against the external engines — the
  recursive CTEs here are depth-bounded to five hops as of v0.5.0
  (`neighbors_one_hop` through `neighbors_five_hop`; v0.1.0 covered one
  hop only, each later version added one more).
- Not a promotion of `three_hop_neighbors`/`four_hop_neighbors`/
  `five_hop_neighbors` into `bench_support` — all three stay private to
  `benches/external_db.rs`, this benchmark's own file, matching this
  project's "no abstraction before two real call sites" convention (see
  the v0.3.0/v0.4.0/v0.5.0 notes above). Composing a third-through-fifth
  hop generically inside `bench_support` itself, alongside
  `two_hop_neighbors`, remains a separate, unmade decision.
...
- `STORAGE-013-FR-010` (v0.4.0): the four-hop graph shape is the same
  ...
  all three engines, same parameter placeholder convention as FR-005/
  FR-008/FR-009.
- `STORAGE-013-FR-011` (v0.5.0): the five-hop graph shape is the same
  `WITH RECURSIVE` traversal, extended one hop deeper again (`WHERE
  hop.depth < 5` in the recursive term, `WHERE depth = 5` in the final
  `SELECT`), matching `benches/external_db.rs`'s own private
  `five_hop_neighbors` — the deduplicated union of `neighbors(n)` for
  every `n` in `four_hop_neighbors(id)` — identical in structure across
  all three engines, same parameter placeholder convention as FR-005/
...
- `cargo bench --features research,external-db-bench --bench external_db`
  runs all seven workload groups (`get`, `scan_ages`, `neighbors_one_hop`,
  `neighbors_two_hop`, `neighbors_three_hop`, `neighbors_four_hop`,
  `neighbors_five_hop`) against all four systems (`production`, `sqlite`,
...
- Six-or-more-hop traversal against the external engines is unmeasured
  — the recursive CTEs here are depth-bounded to 5 hops as of v0.5.0.
  The v0.4.0 crossover (Postgres closest at 1,000,000 records) did
  **not** hold at v0.5.0: SQLite was closest again at 1,000,000 records,
  while a *different* crossover (Postgres closest) appeared at 100,000
  records instead — see `RESULTS.md`'s `### Graph traversal, five-hop`
  subsection. The underlying Postgres query plan has been stable at
  every depth and size tested since v0.2.0 (confirmed again via
  `EXPLAIN (ANALYZE, BUFFERS)`); what's unstable is how much that plan's
  buffer I/O grows per added hop at a given size, which isn't itself a
  predictable function of table size. Given two demonstrated reversals
  in a row, extrapolating "which engine is closest" beyond five hops
  isn't safe — DuckDB's own margin against `ProductionStore`, by
  contrast, *has* narrowed cleanly and monotonically at every hop
  through five, and that trend may be safer to extrapolate.
- Whether `three_hop_neighbors`/`four_hop_neighbors`/`five_hop_neighbors`
  (or an N-hop generalization) belongs in `bench_support` alongside
  `two_hop_neighbors` is an open, deliberately unmade decision — see the
  v0.3.0/v0.4.0/v0.5.0 Non-goals bullet above. A second real call site
  (e.g. a genuine in-repo `neighbors_three_hop` benchmark) would be the
  natural trigger to revisit it.
...
- 0.5.0 (2026-09-01): Added `neighbors_five_hop` (`STORAGE-013-FR-011`)
  — the depth-4 recursive CTE extended one hop deeper, against a new
  private, bench-target-local `five_hop_neighbors`. Real numbers in
  `RESULTS.md`'s new `### Graph traversal, five-hop` subsection. The
  v0.4.0 Postgres/SQLite crossover reversed rather than persisted:
  SQLite was closest again at 1,000,000 records, while a different
  crossover appeared at 100,000 records instead. Confirmed via a fresh
  `EXPLAIN (ANALYZE, BUFFERS)` pass that Postgres's query plan itself is
  unchanged at every depth/size tested since v0.2.0 — the volatility is
  in per-hop buffer-I/O growth, not in which plan runs. No `src/`
  changes, no new ADR — same "completes already-accepted scope"
  reasoning as v0.2.0/v0.3.0/v0.4.0.
