...
- **Companion record blob** (or just **the blob**): the new sibling file
  ...
  (`ages.mmap` → `ages.mmap.records`): a 12-byte header (`DOGBLOB\0`
  magic, `u32` little-endian blob version, currently `1`) followed by
  the `bincode` encoding of `{ records: Vec<DogRecord>, edges: Vec<(Uuid,
  ...
  live age.
- **Current blob**: a blob whose bytes equal, exactly, the encoding of
  the record set a caller just handed `open`.
...
- `STORAGE-014-FR-001` (design `PORTABILITY-FR-001`): `create(records,
  ...
  record set: after `MmapAgeStore::open` succeeds, the blob is read once
  and compared byte-for-byte with the caller's set's encoding; if it is
  missing, unreadable, or different, it is (re)written. The comparison
  runs after the ages file opens so an ages-file error never clobbers a
  valid blob for a different dataset. A pre-`STORAGE-014` directory
  (ages file only) is thereby upgraded on its first `open`, no
  migration step.
...
- `STORAGE-014-FR-005` (design `PORTABILITY-FR-005`): a missing, non-blob
  (wrong magic or shorter than a header), incompatible-version, or
  non-decoding blob at `open_portable` time is one new, distinctly-named
  variant, `DurabilityError::RecordBlobUnreadable { path, cause }`, with
  `cause` naming which of those it is. Never `InvalidMagic`/
...
- `src/durability/record_blob.rs` (new, `pub(crate)`, unconditional —
  ...
  (`Serialize`/`Deserialize`/`PartialEq`) with `encode() ->
  EncodedRecordBlob` and `read(&Path) -> Result<Self>`;
  `EncodedRecordBlob` (the header+body image) with `write(&Path)` (the
  temp-then-rename install) and `is_current_at(&Path) -> bool` (one
  `fs::read`, byte comparison). `encode` is split from `write` so
  `ProductionStore` serializes *before* moving `records`/`edges` into
  `MmapAgeStore` by value — no clone of the record set to persist it.
...
- `src/production.rs`: `create` = encode → `MmapAgeStore::create` →
  blob write; `open` = encode → `MmapAgeStore::open` → compare → rewrite
  if not current; `open_portable(path)` = `RecordBlob::read` →
  `MmapAgeStore::open`. Module docs gain a "File portability" section.
...
- After any successful `create` or `open`, the blob is current for the
  ...
  untouched.
- Blob bytes are deterministic for equal inputs (`bincode`, caller
  order preserved), which is what makes the byte comparison in
  `STORAGE-014-FR-001` equivalent to a decoded `PartialEq` comparison —
  at one file read and no second record set allocated.
...
- No `unwrap`/`expect` outside `#[cfg(test)]` — the header's version
  bytes are read by bounds-checked indexing after the length check, the
  same pattern `read_wal_entries` uses.
...
- `open` with an unchanged record set leaves the blob's bytes and mtime
  alone; `open` with a changed set (a record and an edge added)
  rewrites it, and a subsequent `open_portable` sees the new record's
  `breed` and edge.
...
- The record blob's own unit tests cover: path derivation; round trip
  ...
  header); shorter-than-header; version mismatch; truncated body — each
  as `RecordBlobUnreadable` with the expected `cause` substring; the
  temp-then-rename replacement consuming a stale temp file; and
  `is_current_at` detecting a removed record or edge.
...
- `cargo test` (default features) and `cargo test --all-features`: the
  9 `record_blob` tests and 5 new `production` tests above, plus every
  pre-existing test, passing.
...
- **`open`'s comparison is not free — measured, not the design's
  "expected zero."** The design assumed `open` could piggyback on the
  rewrite decision `MmapAgeStore::open` already makes internally. That
