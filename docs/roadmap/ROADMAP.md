...
already exists if that's ever revisited). No implementation is
authorized yet — this is a design proposal only, awaiting the owner's
review before any code is written or any dependency actually added to
`Cargo.toml`.
