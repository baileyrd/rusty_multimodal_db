...
proposed, with no changes requested; implementation remains a separate,
not-yet-started unit, extending `SERVER-001` and `STORAGE-011`/
`STORAGE-012` with new FRs (or registering a dedicated spec) before any
server or storage-layer code lands, matching how `SERVER-AUTH` followed
`SERVER-AUTH-DESIGN`'s own acceptance.
