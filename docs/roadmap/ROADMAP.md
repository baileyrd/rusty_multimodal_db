...
- A wire-protocol version field or hello handshake, and a `bincode` 2.x
  ...
  *encoding* under the `Request`/`Response` shape and the three blob
  ...
  negotiation is now **designed, not built**, by
  `SERVER-PROTOCOL-VERSION-DESIGN`/ADR-0022 (Accepted): a
  `PROTOCOL_VERSION` constant, an optional first-frame `Hello`, and
  append-only compatibility rules; 2.x and the `Uuid` encoding stay
  as they were, now with a version to be introduced under.
