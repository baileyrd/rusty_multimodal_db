...
use crate::durability::record_blob::{
    encode_image, parse_header, EncodedRecordBlob, Fnv1a64, HEADER_LEN,
};
...
where
    Id: Serialize,
{
    pub(crate) fn new(edges: &'a [(Id, Id)]) -> Self {
        Self { edges }
    }
    ...
    /// [`HEADER_LEN`]-byte read — never a full serialization to a buffer,
    /// never a read of the body. A missing, short, foreign (`GENBLOB\0`
    /// included), or wrong-version file counts as "not current" so that
    /// `open` heals it from the caller-supplied truth — including a
    /// directory written before the edge blob existed. A serialization
...
/// incompatible version, its body doesn't decode, or the body's bytes
/// don't hash to the fingerprint the header claims — maps to
/// [`DurabilityError::RecordBlobUnreadable`], naming the edge blob path
/// and the specific cause. Never a panic, never a silently empty list
/// (`SYMPORT-FR-005`).
///
/// # Errors
///
/// Returns [`DurabilityError::RecordBlobUnreadable`] as described above.
pub(crate) fn read<Id>(path: &Path) -> Result<Vec<(Id, Id)>, DurabilityError>
where
    Id: DeserializeOwned,
{
...
mod tests {
    ...
    fn encode_then_read_round_trips_uuid_pairs_in_order() {
        ...
        let loaded: Vec<(Uuid, Uuid)> = read(&path).unwrap();
    ...
    fn a_reordered_or_different_edge_list_is_not_current() {
        ...
        reordered.swap(0, 2);
        assert!(!EdgeBlob::new(&reordered).is_current_at(&path));
        assert!(!EdgeBlob::new(&edges[..2]).is_current_at(&path));
        let mut flipped = sample();
        flipped[0] = (flipped[0].1, flipped[0].0);
        assert!(!EdgeBlob::new(&flipped).is_current_at(&path));
        let _ = std::fs::remove_dir_all(&dir);
    }
    ...
    fn an_empty_edge_list_round_trips() {
        ...
        assert!(EdgeBlob::new(&edges).is_current_at(&path));
        assert!(!EdgeBlob::new(&sample()).is_current_at(&path));
        let _ = std::fs::remove_dir_all(&dir);
    }
    ...
    fn a_generic_record_blob_at_the_edges_path_is_a_magic_error() {
        ...
        // too: only the magic tells the two apart, and it must.
        let edges = sample();
        super::super::record_blob::GenericRecordBlob::new(&edges)
            .encode()
            .unwrap()
            .write(&path)
            .unwrap();
